...
const LetterGlitch: React.FC<LetterGlitchProps> = ({
  glitchColors = ['#1e293b', '#0ea5e9', '#0d9488', '#4f46e5'],
  className = '',
  glitchSpeed = 50,
  ...
  useEffect(() => {
    ...
    // Visibility-based pause: stop animation when off-screen to save resources
    let isVisible = true;
    isRunning.current = true;

    const maxFpsInterval = 1000 / 20; // Cap to 20 FPS
    let lastFrameTime = 0;

    const tick = (now: number) => {
      if (!isRunning.current || !isVisible) return;

      // FPS throttling - skip frame if too soon
      if (now - lastFrameTime < maxFpsInterval) {
    ...
    // Observe visibility — pause when off-screen, resume when on-screen
    const observer = new IntersectionObserver(
      ([entry]) => {
        isVisible = entry.isIntersecting;
        if (isVisible && isRunning.current) {
